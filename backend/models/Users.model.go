...
const (
	UserStatusDeleted  int8 = 0
	UserStatusInactive int8 = 9
	UserStatusActive   int8 = 10
)
...
func FindUserByPasswordReset(dest interface{}, token string) *gorm.DB {
	return database.GDB.Where("password_reset_token = ?", token).First(dest)
}

func FindUserById(dest interface{}, idAccount interface{}) error {
	err := database.GDB.Where("id_account = ?", idAccount).First(dest).Error
...
func FindUser(dest interface{}, conds ...interface{}) *gorm.DB {
	return database.GDB.Model(&User{}).Select("id_account", "identity_number", "username", "full_name", "email", "phone_number", "date_of_birth", "status_account", "login_ip", "login_attempts", "login_time", "created_at", "updated_at").Order("username").Find(dest, conds...)
}
