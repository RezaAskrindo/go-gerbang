...
func FindLogger(dest *[]Logger, service, method, path, status string, from, to time.Time) *gorm.DB {
	query := `
			SELECT id, level, service, method, path, user_auth, status, duration, fields, timestamp FROM loggers 
			WHERE service = ?
			AND method = ?
			AND path = ?
			AND status = ?
