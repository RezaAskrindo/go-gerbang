...
func GetImageEmail(sender string, provider string) string {
	switch provider {

	case "Resend":
		for _, key := range handlers.GetEmailResendConfig() {
			if key.Sender == sender && key.ImageElement != nil {
				return *key.ImageElement
			}
		}

	case "SMTP":
		for _, key := range handlers.GetEmailSMTPConfig() {
			if key.Sender == sender && key.ImageElement != nil {
				return *key.ImageElement
			}
		}
